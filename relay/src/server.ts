...
export function createRelayServer(options: RelayServerOptions = {}) {
  const maxPatchHistory = options.maxPatchHistory ?? MAX_PATCH_HISTORY;
  ...
  const maxConnectionsPerIp =
    options.maxConnectionsPerIp ?? MAX_CONNECTIONS_PER_IP;
  const maxChannelsPerConnection =
    options.maxChannelsPerConnection ?? MAX_CHANNELS_PER_CONNECTION;
  ...
  wss.on("connection", (socket, request) => {
    ...
    socket.on("message", (data, isBinary) => {
      ...
      const patch = Buffer.isBuffer(data)
        ? data
  ...
  function rememberPatch(channelId: string, patch: Buffer): void {
    ...
    while (patches.length > maxPatchHistory) {
      patches.shift();
    }
