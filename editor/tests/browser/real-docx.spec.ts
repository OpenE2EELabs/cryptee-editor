...
test("opens an encrypted DOCX through the real CryptPad ONLYOFFICE runtime", async ({
  page,
  baseURL,
}) => {
  ...
  await expect(
    page.getByText("ONLYOFFICE vendor assets are present"),
  ).toHaveCount(0);
  ...
  await expect(page.locator(".licensebar")).toContainText(
    "github.com/OpenE2EELabs/cryptee-editor",
  );
});

async function encryptForProtocol(
